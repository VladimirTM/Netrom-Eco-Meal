...
| `IPackageRepository` / `PackageRepository` | `GetAllAsync`, `GetPagedAsync(search, businessId?, packageTypeId?)`, `GetByIdAsync`, `GetByIdsAsync`, `AddAsync`, `DeleteAsync`, `SaveChangesAsync` | `GetByIdsAsync` is the batch-load path `PlaceOrderAsync` uses to re-validate every package referenced in the submitted cart lines in one query. `CartService` restores its own localStorage-persisted cart directly against `EcoMealDbContext` via `IDbContextFactory`, not through this repository — see §5 |
