...
| `PackageController` | `GetAllAsync`, `GetPagedAsync`, `GetByIdAsync`, `AddAsync`, `UpdateAsync`, `DeleteAsync`, `DuplicateManyAsync`, `AdjustQuantityManyAsync`, `ExtendPickupWindowManyAsync`, `GetForAnalyticsAsync` → `Unauthorized()` on a non-admin's missing/foreign `businessId` |
