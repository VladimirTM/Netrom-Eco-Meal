...
**Stack:** ASP.NET Core 10 · Blazor Server (interactive server render mode) · Entity Framework Core · PostgreSQL (Npgsql) · ASP.NET Identity (cookie auth) · QRCoder · Stripe Checkout (`Stripe.net`) · `Microsoft.Extensions.AI` + `OllamaSharp` (self-hosted Ollama)
...
│   └── *.cs                      # Business logic, DI-registered Scoped (3 aren't — 2 BackgroundServices, 1 Singleton, see §5)
...
├── Models/                      # PaginatedList<T>, Debouncer, GeoDistance, BusinessHoursStatus, LeaderboardEntry (Debouncer is frontend-facing, see FRONTEND_ARCHITECTURE.md)
...
```
Extends Identity's built-in user with the two fields Identity doesn't provide: a display name and (Phase 11) an opt-in flag for the `/impact` leaderboard. Everything else (email, password hash, roles) comes from `IdentityUser`/`IdentityDbContext` for free. `ShowOnLeaderboard` defaults `false` — a customer flips it from a toggle right on `/impact` (`ImpactService.SetMyOptInStatusAsync`, via `UserManager<ApplicationUser>`, not a repository — there's no `ApplicationUser`-specific repository in this app, Identity's own store already covers CRUD). `OrderRepository.GetTopRescuersAsync` filters on it directly, so a customer who never opts in never appears on the leaderboard, full stop — not even as an anonymized row.
...
```
Unique index on `(BusinessId, UserId)` — resubmitting via `ReviewService.SubmitAsync` updates the existing row in place (and bumps `CreatedAt`) rather than creating a duplicate. Gated on `IOrderRepository.HasCompletedOrderAsync(userId, businessId)` — a customer must have at least one `Completed` order with that business before they can review it. `PackageId` narrows what the review is about without changing that scope — still one review per `(BusinessId, UserId)`, just optionally pointed at one of the packages the reviewer actually completed an order for (`IOrderRepository.GetCompletedPackagesAsync`, the picker source for `ReviewContext.ReviewablePackages`). `SubmitAsync` re-validates a submitted `PackageId` against that same set and silently drops it if it isn't in there, rather than throwing — a stale client-side pick (e.g. the picker rendered before a since-cancelled order) degrades to an untagged review instead of failing the whole submission. `OnDelete(DeleteBehavior.SetNull)` on the FK means deleting a package un-tags any review that pointed at it instead of deleting the review.
...
```
One row per browser/device a user has enabled push on — a user signed in on several devices gets several rows, and each gets its own push. `Endpoint` (the push service's own per-subscription URL, e.g. `https://fcm.googleapis.com/fcm/send/...`) carries a unique index; `PushSubscriptionRepository.AddAsync` re-points an existing row to the calling user rather than fail that index if the same browser endpoint resubscribes under a different account (a shared browser, mainly). Cascade-deleted with its `User`. `PushSubscriptionService.SubscribeAsync`/`UnsubscribeAsync` are scoped to the signed-in caller's own rows (`UnsubscribeAsync` loads the caller's own subscriptions and only removes a matching `Endpoint`, since the repository's own `RemoveByEndpointAsync` is intentionally unscoped — it doubles as the prune path below); `SendToUserAsync(userId, message, url)` is the one exception, called by `NotificationService.CreateAsync` to push to *someone else*, the same relationship `INotificationService.CreateAsync` itself has to its own read methods. A subscription the push service reports as gone (HTTP 404/410 from `IWebPushGateway.SendAsync`) is deleted automatically on the next send attempt — no separate cleanup sweep needed, since a dead endpoint only ever surfaces when something actually tries to use it.
...
| `IBusinessTypeRepository` / `BusinessTypeRepository` | `GetAllAsync` (ordered by `Name`), `GetByIdAsync`, `IsInUseAsync`, `AddAsync`, `DeleteAsync`, `SaveChangesAsync` | Phase 11 gave this a real write side, backing the admin-only `/types` page — see `IBusinessTypeService` below for why `IsInUseAsync` exists at all |
...
| `IOrderRepository` / `OrderRepository` | `GetAllAsync`, `GetByUserIdAsync`, `GetByBusinessIdAsync`, `GetPagedByUserIdAsync`, `GetPagedForManagementAsync(search, businessId?, status?)`, `GetInRangeAsync(businessId?, from?, to?)`, `GetByIdAsync`, `HasCompletedOrderAsync`, `GetCompletedPackagesAsync`, `GetTotalWeightSavedKgAsync`, `GetTopRescuersAsync(rangeStart, rangeEndExclusive, take)`, `GetStalePendingOrdersAsync`, `GetOverduePickupOrdersAsync`, `GetPickupReminderCandidatesAsync`, `GetPendingQuantitiesByPackageIdsAsync`, `AddAsync`, `DeleteAsync`, `SaveChangesAsync` | The busiest repository — see `OrderService` in §5 for how its methods compose. `GetPagedForManagementAsync`'s order-number search strips a leading `#` and substring-matches the plain integer (`EF.Functions.ILike(o.OrderNumber.ToString(), ...)`) because Npgsql can't translate the zero-padded `ToString("000")` overload into SQL — it also strips any leading zeros left after the `#`, so searching the padded number shown on screen (`#008`) still matches. `GetInRangeAsync` is unpaginated and date-bounded — it feeds both the CSV export and the dashboard trend chart (see §6 and FRONTEND_ARCHITECTURE.md §11). `GetPendingQuantitiesByPackageIdsAsync` is a `GroupBy`/`Sum` over `OrderPackages` for a batch of package IDs — the same Pending-reservation shape `PlaceOrderAsync`'s `pendingElsewhere` check computes per-package, exposed in bulk for display (§5). `GetCompletedPackagesAsync` is the distinct-packages-per-`Completed`-order query backing `ReviewContext.ReviewablePackages` (§3, §5). `GetTopRescuersAsync` (Phase 11) is `GetTotalWeightSavedKgAsync`'s per-user, date-bounded, opt-in-filtered sibling — groups `OrderPackages` by `(Order.UserId, Order.User.Name)`, requiring `Order.Status.Name == Completed`, `Order.CreatedAt` inside the range, and `Order.User.ShowOnLeaderboard`, all server-side (unlike the client-side `Sum` the personal kg-saved stats on `Orders.razor`/`Dashboard.razor` use — see FRONTEND_ARCHITECTURE.md §6/§11) since ranking potentially many users needs the DB doing the grouping, not the app pulling every order into memory |
| `IPackageRepository` / `PackageRepository` | `GetAllAsync`, `GetPagedAsync(search, businessId?, packageTypeId?)`, `GetByIdAsync`, `GetByIdsAsync`, `GetForAnalyticsAsync(businessId?, since)`, `GetNearExpiryUnclaimedAsync(now, closingBefore)`, `GetLiveCandidatesAsync(dietaryTag?)`, `GetMarkdownCandidatesAsync(businessId?, now, closingBefore)`, `GetSellThroughHistoryAsync(businessId, excludePackageId, since)`, `AddAsync`, `DeleteAsync`, `SaveChangesAsync` | `GetByIdsAsync` is the batch-load path `PlaceOrderAsync` uses to re-validate every package referenced in the submitted cart lines in one query — the bulk-action toolbar (§5) reuses it too, to load every selected package before duplicating/adjusting/extending. `GetForAnalyticsAsync` includes each package's `OrderPackages`/`Order`/`Status` graph so the Dashboard can aggregate sell-through and pickup-hour stats without a second round-trip. `GetAllAsync`/`GetPagedAsync` are the two purely-read/display paths (every write re-fetches through `GetByIdAsync`/`GetByIdsAsync` instead) and, since Phase 12, both call `.AsNoTracking()` — without it, `BusinessDetail.razor`'s live-reload re-query kept returning the *first* query's tracked `Package` instances for the lifetime of the circuit (`EcoMealDbContext` is `Scoped` to the whole circuit, not per-request — see §2.2/§5), silently ignoring whatever a different circuit had actually changed in the database via EF's identity map. `CartService` restores its own localStorage-persisted cart directly against `EcoMealDbContext` via `IDbContextFactory`, not through this repository — see §5. `GetLiveCandidatesAsync` (AI Phase 4) is the same "live, unhidden, stock-remaining" shape `GetNearExpiryUnclaimedAsync` (AI Phase 3) uses, minus the closing-window bound, plus an optional `DietaryTags.Contains` filter — feeds `BasketPlannerAgent`'s `search_live_packages` tool (§5), one call per distinct tag/kitchen the model wants to compare. `GetMarkdownCandidatesAsync` (AI Phase 5) is the same shape again but adds `MarkdownDismissedAt == null` and a wider (3-hour, `MarkdownSettings.ClosingWindow`) closing bound — backs the `/packages` 🏷️ badge. `GetSellThroughHistoryAsync` (AI Phase 5) is a different shape from the rest: server-side filtered to one business's own **closed** packages (`PickupEnd < now`) since a lookback date, `.Include(p => p.PackageType)` added on top of `GetForAnalyticsAsync`'s `OrderPackages`/`Order`/`Status` graph, ordered most-recent-first and capped at `MarkdownSettings.MaxHistoryRecords` — feeds `MarkdownPricingAgent`'s `get_sell_through_history` tool via `PackageService.GetMarkdownSuggestionAsync` |
| `IPackageTypeRepository` / `PackageTypeRepository` | `GetAllAsync` (ordered by `Name`), `GetByIdAsync`, `IsInUseAsync`, `AddAsync`, `DeleteAsync`, `SaveChangesAsync` | Same Phase 11 write side and `IsInUseAsync` reasoning as `IBusinessTypeRepository` above, over `Package.PackageTypeId` instead |
| `IReviewRepository` / `ReviewRepository` | `GetAllAsync`, `GetByBusinessIdAsync`, `GetByBusinessIdsAsync`, `GetByUserAndBusinessAsync`, `AddAsync`, `SaveChangesAsync` | `GetByBusinessIdsAsync` is the batch path `Home.razor` uses to load ratings for an entire page of business cards in one query instead of one query per card. `GetByBusinessIdAsync` also `.Include(r => r.Package)` so `BusinessDetail.razor`'s review list can show a package-name pill without a second query |
...
| `IPushSubscriptionRepository` / `PushSubscriptionRepository` | `GetByUserIdAsync`, `AddAsync`, `RemoveByEndpointAsync` | `AddAsync`/`RemoveByEndpointAsync` persist immediately, same shape as `IFavoriteRepository` — no staged-then-saved split. `AddAsync` re-points an existing row to the calling user instead of failing the unique index on `Endpoint` if the same browser subscribes under a different account |
...
| `IImageUploadService` | `ImageUploadService` | No auth check of its own — `BusinessForm.razor`/`PackageForm.razor` are already gated to admin/staff, and `SaveAsync(content, originalFileName, subfolder)` is only ever called from those two forms' `InputFile.OnChange` handlers. Validates the extension against `Constants.ImageUpload.AllowedExtensions` (throws `InvalidOperationException`, shown directly in the form, same convention as `PackageTypeService`'s delete-in-use guard) and streams the file to `wwwroot/uploads/{subfolder}/{new Guid}{ext}` via `IWebHostEnvironment.WebRootPath`, returning that path as the `/uploads/...` URL to store in `Package.ImageUrl`/`Business.ImageUrl` — no schema change, it's just a different way of populating the same text column a pasted URL always could. The one real gotcha: `Program.cs`'s `app.MapStaticAssets()` only serves the build-time static-web-assets manifest, so a file written here after the app has started would otherwise 404 (or get swallowed by the Blazor Server fallback route) despite sitting right there on disk — a separate `app.UseStaticFiles(...)` scoped to `wwwroot/uploads` (registered before `MapStaticAssets`) covers it. `docker-compose.test.yml` mounts that directory as a named volume so uploads survive a rebuild |
| `IPackageAiAssistant` | `PackageAiAssistant` (`Services/AI/`) | **Phase 1.** One method, `DraftDescriptionAsync(name, packageTypeName, dietaryTags)`, over an injected `IChatClient?` (`Microsoft.Extensions.AI`, backed by `OllamaSharp`'s `OllamaApiClient` against a self-hosted Ollama instance — no paid/hosted API anywhere in this path). No tool-calling, no JSON schema — a single `GetResponseAsync` call with a plain prompt, then `.Text.Trim()`. `chatClient` defaults to `null` in the constructor and is only ever resolved from DI when `Ollama:BaseUrl` is configured (`Program.cs`), so `DraftDescriptionAsync` throws a friendly `InvalidOperationException` ("AI features aren't available yet — set Ollama:BaseUrl to enable them.") instead of a null-reference — same degrade-gracefully convention as `StripeGateway.EnsureConfigured`. A network/model failure from Ollama itself is also caught and rewrapped into a friendly `InvalidOperationException` rather than surfacing a raw `OllamaSharp`/HTTP exception to the manager |
| `ISearchIntentParser` | `SearchIntentParser` (`Services/AI/`) | **Phase 2.** `ParseAsync(utterance, previousIntent?)` turns a shopper's free-text query ("vegan dinner under 30 lei, closing soon") into a `SearchIntent` (`Models/SearchIntent.cs`: `Keywords`/`DietaryTag`/`MaxPrice`/`ClosingSoon`/`NearMe`) that `Home.razor` applies against `BusinessController.GetPagedAsync`'s existing filters. Unlike `IPackageAiAssistant`, this one uses `ChatOptions.ResponseFormat = ChatResponseFormat.ForJsonSchema<RawSearchIntent>()` with `Temperature = 0` for deterministic structured output; the response is deserialized with `AIJsonUtilities.DefaultOptions` (camelCase, case-insensitive — has to match the same options `ForJsonSchema<T>` used to name the schema's properties, or a plain `Deserialize<T>()` misses every field). The raw deserialized shape is a private `RawSearchIntent`, kept separate from the public `SearchIntent` so nothing unvalidated can leak through: `Validate()` re-checks `DietaryTag` against `Constants.DietaryTags.All` case-insensitively (a fabricated tag is dropped to `null`, never trusted) and clamps `MaxPrice` to positive values. `previousIntent`, when given, is serialized into the prompt as explicit context ("the shopper already has these filters — treat this as a refinement") rather than replayed as chat history, so a follow-up like "cheaper" adjusts the prior turn's price ceiling instead of starting over — a local model is more likely to lose track of replayed history than a hosted frontier model would be. Same null-`chatClient`/friendly-error/rewrapped-exception conventions as `IPackageAiAssistant` above |
| `INearExpiryNudgeComposer` | `NearExpiryNudgeComposer` (`Services/AI/`) | **Phase 3.** One method, `ComposeAsync(packageName, businessName, quantity, timeUntilClose, matchedDietaryTag?)`, over the same injected `IChatClient?` as `IPackageAiAssistant` above — pure text generation, no tool-calling, no JSON schema. Same null-`chatClient`/friendly-error/rewrapped-exception conventions; `matchedDietaryTag`, when given, is folded into the prompt as an extra instruction to call out that the package matches something the customer has ordered before, rather than a separate templated sentence |
| `IBasketPlannerAgent` | `BasketPlannerAgent` (`Services/AI/`) | **Phase 4.** `ProposeBasketAsync(peopleCount, budget, dietaryTag?)` — the first AI feature that needs Ollama's tool-calling to hold up, not just plain text or `ForJsonSchema` output. Split into two chat turns rather than one, since a free local model isn't reliably good at combining tool-calling and strict JSON-schema output in the same turn: turn 1 wraps the injected `IChatClient?` with `.AsBuilder().UseFunctionInvocation().Build()` and gives it a genuine `AIFunctionFactory`-built `search_live_packages` tool (a local closure over `IPackageService.GetLiveCandidatesAsync`, re-validating the model's own `dietaryTag` argument against `Constants.DietaryTags.All` the same way `SearchIntentParser` validates a free-text extraction) — the model calls it as many times as it wants, populating a `candidatePool` dictionary as a side effect; turn 2 goes back to the *plain* `chatClient` (tools switched off) with `ChatOptions.ResponseFormat = ChatResponseFormat.ForJsonSchema<RawBasketPlan>()`, asking it to commit to a final basket referencing only `packageId`s the tool actually returned. `BuildValidatedPlan` then re-validates every field against `candidatePool` instead of trusting the JSON: an unresolvable `packageId` is dropped, `Quantity` is clamped to the real `Package.Quantity`, and `LineTotal`/`TotalPrice` are recomputed from the real `Package.Price` — never the model's numbers. Two Eco Meal-specific rules are enforced here too, regardless of whether the prompt already asked for them: `CartService`'s single-business-per-basket constraint (if the survivors still span more than one `BusinessId`, only the higher-`LineTotal`-sum kitchen's items are kept, and the customer-facing `Explanation` gets a note saying so) and a budget ceiling (items are dropped from the end of the list until the total fits `budget`, in case the model's own arithmetic was off) — and if that budget trim empties the basket entirely, the model's own `Explanation` text (which was describing the now-gone items) is discarded in favor of a fixed "nothing fits" message, rather than shown stale next to an empty list. Same null-`chatClient`/friendly-error conventions as every other `Services/AI/` class |
| `IMarkdownPricingAgent` | `MarkdownPricingAgent` (`Services/AI/`) | **AI Phase 5**, the highest-value/highest-risk `Services/AI/` class so far — a bad suggestion costs a manager real revenue, so it's never applied automatically, only shown as a dismissable suggestion. `SuggestMarkdownAsync(package, history)` takes an already-fetched, already-authorized `List<PackageSellThroughRecord>` (built by `PackageService.GetMarkdownSuggestionAsync` above) rather than querying anything itself, keeping this class a pure AI-orchestration layer with no data-access/authorization concerns — unlike `IBasketPlannerAgent`, which owns its own `IPackageService`-backed tool. Same two-turn shape as `IBasketPlannerAgent` though: turn 1 wraps `IChatClient?` with `.UseFunctionInvocation()` and gives it a `get_sell_through_history` tool that's just a closure returning the pre-fetched `history` list (no fresh DB query per call, since it's already business-scoped and capped at `MarkdownSettings.MaxHistoryRecords`); turn 2 goes back to the plain `chatClient` with `ChatOptions.ResponseFormat = ChatResponseFormat.ForJsonSchema<RawMarkdownSuggestion>()`. `BuildValidatedSuggestion` re-validates the model's own numbers afterward: a `SuggestedPrice` that isn't strictly below the package's current price returns `null` (no cut warranted), and anything below `MarkdownSettings.MinPriceFraction` (30%) of the current price is clamped up to that floor rather than trusted as-is — so a bad or fabricated response can only be discarded or clamped, never invent a price hike or an absurd markdown. Same null-`chatClient`/friendly-error/rewrapped-exception conventions as every other `Services/AI/` class |
| `INearExpiryNudgeService` | `NearExpiryNudgeService` | **Phase 3.** `SweepAsync()` — called by `NearExpiryNudgeSweepService` (§8), not by any controller. Loads packages closing within `PackageNudgeSettings.ClosingSoonWindow` (30 minutes) that still have stock and haven't been nudged yet (`IPackageRepository.GetNearExpiryUnclaimedAsync`), then for each one builds an audience from `IFavoriteRepository.GetFavoritingUsersAsync` ∪ `IOrderRepository.GetPastCustomersAsync` (favoriters and anyone with a `Completed` order at that business) — an AI-scored trigger instead of a hand-coded rule: unlike `PackageService.NotifyFavoritingCustomersAsync`, a favoriter only ever hears about a package that's actually at risk, not every stock change. The audience is grouped by whichever of the package's `DietaryTags` (if any) also shows up in that customer's own `GetCompletedPackagesAsync` history at the same business, so `INearExpiryNudgeComposer` is called once per distinct match (not once per customer) and the resulting copy reaches every matching customer through the existing bell (`INotificationService`) + email (`IAppEmailSender`, `EmailTemplateBuilder`) channels. Marks `Package.NearExpiryNudgeSentAt` whether or not there was anyone to notify, so a package is only ever considered once |
...
| `INotificationService` | `NotificationService` | Scoped to the signed-in user for every read method; `CreateAsync(userId, message, url?)` is the one exception — other services call it to notify *someone else* (e.g. `OrderService` notifying a business's manager about a new order). Also fires a best-effort web push via `IPushSubscriptionService` — see §3 PushSubscription |
| `IPushSubscriptionService` | `PushSubscriptionService` | `SubscribeAsync`/`UnsubscribeAsync` scoped to the signed-in caller, same shape as `IFavoriteService`; `GetPublicKey()` (sync, no auth) hands the frontend the VAPID public key or `null` if push isn't configured; `SendToUserAsync(userId, message, url?)` is `NotificationService`'s one caller — see §3 PushSubscription |
| `IWebPushGateway` | `WebPushGateway` | Thin wrapper over the `WebPush` NuGet package (VAPID-signed Web Push), same shape/degrade-gracefully pattern as `IStripeGateway` — except unlike Stripe/SMTP this needs no external account at all, just a self-generated EC key pair (`WebPush.VapidHelper.GenerateVapidKeys()`), so it ships pre-configured for local dev (see §10). Reads `WebPush:PublicKey`/`PrivateKey`/`Subject` from `IConfiguration`. `SendAsync` returns `false` only on an HTTP 404/410 from the push service (subscription confirmed gone) — any other failure is logged and swallowed, same "never blocks the underlying transition" contract as `IAppEmailSender` |
| `IPackageService` | `PackageService` | CRUD, admin-or-own-business-manager on writes (`EnsureCanManageBusinessAsync`). `AddAsync`/`UpdateAsync` also notify (bell + email) every customer who's favorited the package's business when it's created in stock or restocked from `0` — see §3 Package. `DuplicateManyAsync`/`AdjustQuantityManyAsync`/`ExtendPickupWindowManyAsync` back the `/packages` bulk-action toolbar (Phase 8), each authorizing every affected package's business via `EnsureCanManageBusinessesAsync` (a thin loop over the single-package check) before mutating. `GetForAnalyticsAsync(businessId?, since)` backs the Dashboard's Business Analytics card — admin-only for a `null` businessId, otherwise requires `IsStaffAsync` on the requested one, same scoping shape as `OrderService.ResolveManagerBusinessIdAsync`. **Phase 9**: `HideAsync`/`UnhideAsync` — same `EnsureCanManageBusinessAsync` authorization as every other write, so either an admin or the package's own business staff can moderate it; both log to `IAuditLogService`. **AI Phase 5**: `GetMarkdownCandidatesAsync(businessId?)` — same admin-null/staff-scoped authorization shape as `GetForAnalyticsAsync`, delegates to `IPackageRepository.GetMarkdownCandidatesAsync` (live, unhidden, stock remaining, `PickupEnd` within `MarkdownSettings.ClosingWindow`, not yet dismissed) to back the `/packages` 🏷️ badge. `GetMarkdownSuggestionAsync(packageId)` — `EnsureCanManageBusinessAsync` on the package's own business, then builds this business's own comparable sell-through history (`IPackageRepository.GetSellThroughHistoryAsync`, closed packages from the last `MarkdownSettings.LookbackDays`, mapped to `PackageSellThroughRecord`) and hands both to `IMarkdownPricingAgent.SuggestMarkdownAsync` — the authorization/data-fetching stays here, deliberately kept out of the AI class itself (see below). `DismissMarkdownSuggestionAsync(packageId)` sets `Package.MarkdownDismissedAt`, same authorization |
...
| `PushSubscriptionController` | `GetPublicKey()` → the VAPID public key or `null`, `SubscribeAsync(endpoint, p256dh, auth)` → `NoContent()`/`Conflict()`, `UnsubscribeAsync(endpoint)` → `NoContent()` |
...
Same shape again — `IServiceScopeFactory` + `PeriodicTimer`, one `try/catch`-wrapped tick body, registered via `AddHostedService<NearExpiryNudgeSweepService>()`. Runs every `PackageNudgeSettings.SweepInterval` (5 minutes, same as `OrderLifecycleSweepService`) and calls `INearExpiryNudgeService.SweepAsync()` (§5), logging `"Sent {Count} near-expiry nudge(s)."` when it actually notified anyone. **Phase 3**: no per-candidate `try/catch` inside `SweepAsync` itself — if `INearExpiryNudgeComposer` throws (most commonly because `Ollama:BaseUrl` isn't configured), the whole tick's exception is caught here, logged, and nothing gets marked `NearExpiryNudgeSentAt`, so the same candidates are simply reconsidered on the next tick once Ollama is reachable again — deliberately simpler than `OrderLifecycleSweepService`'s per-call granularity, since a local model being unavailable is an all-or-nothing condition for this sweep, not a per-order edge case.
...
2. **Seed admin & demo accounts** — reads `SeedAdmin:Email`/`SeedAdmin:Password` from configuration for the admin; if either is missing, logs a warning and skips (no admin account is seeded, the app still runs). Unconditionally creates `demo.customer@ecomeal.local`, `demo.customer2@ecomeal.local`, `demo.customer3@ecomeal.local`, `demo.manager@ecomeal.local`, and `demo.manager2@ecomeal.local` (fixed passwords, not configuration-gated like the admin account, since they carry no real data) — the second demo manager exists purely so a fresh database demonstrates both directions of the `BusinessStaff` many-to-many (point 5 below), and the two extra demo customers exist purely for the Phase 11 `/impact` leaderboard (point 10 below). All six use the same find-or-create-and-assign-role helper.
...
4. **Demo businesses & packages** — a fixed set of World-Cup-themed Timișoara businesses/packages with **hardcoded GUIDs**, reconciled against what's already in the DB rather than blindly re-inserted:
   ...
   - A package's `PickupStart`/`PickupEnd` are only refreshed (advanced to "today") if the existing window has **already expired** — so the storefront always opens with live, orderable packages on any given day, without resetting `Quantity` (which reflects real orders placed against it) or touching a still-valid future window. Skipped entirely once `TemplateId` is set (see point 6) — a template-owned package is left to expire naturally rather than fought over by two refresh mechanisms. Three packages are the exception to the "anchored to today" rule, each anchored to `DateTime.UtcNow` itself instead of a fixed hour so they're always inside their respective "closing soon" window right after a fresh seed, whatever time of day the app starts: "Stoppage Time Pastry Box" (Phase 10, `now - 30min` to `now + 25min`) demos `Home.razor`'s one-hour closing-soon badge/sort, "Late Save Meal Box" (AI Phase 3, `now - 15min` to `now + 20min`) demos `NearExpiryNudgeSweepService` (§8) — it's at the same business (Stadionul de Gusturi) the demo customer both favorites and has a `Completed` `mealBox` order from, so on a fresh seed the AI nudge actually calls out the dietary-tag match — and "Away Day Meal Box" (AI Phase 5, `now - 2h` to `now + 90min`) demos the `/packages` 🏷️ markdown suggestion: same business and `mealBox` type as the historical `hist1`/`hist3` packages from point 8 below, but priced at 12.99 RON against their real 9.50–9.99 RON sell-through, so `MarkdownPricingAgent`'s tool call has genuine comps that favor a cut. Whenever any of the three windows gets refreshed, `NearExpiryNudgeSentAt`/`MarkdownDismissedAt` are reset to `null` alongside `PickupStart`/`PickupEnd` — otherwise a package that was already nudged/dismissed on a previous run would look "already handled" even though its window is brand new.
   - `WeightKg`, `DietaryTags`, and `Business.Latitude`/`Longitude` are **backfill-only** — only set if currently `0`/empty/`null` — since the seeder can't distinguish "never set" from "an admin/manager deliberately cleared it," so it defaults to filling in demo data either way rather than guessing. `DietaryTags` defaults per `PackageTypeId` (e.g. every veggie box gets `Vegan`/`Vegetarian`/`Gluten-Free`/`Dairy-Free`), plus two Phase 10 per-package overrides ("World Cup Meal Box" → adds `Halal`, "Trophy Pastry Box" → adds `Contains Nuts`) so every tag in `Constants.DietaryTags.All` has at least one real package behind it for the home page's dietary filter to demo against.
...
10. **Phase 11 leaderboard demo data** (`SeedLeaderboardDemoDataAsync`) — opts the primary demo customer and `demo.customer2` into the `/impact` leaderboard (`ShowOnLeaderboard = true`, set unconditionally every run since these are fixed demo identities, not real user preferences), and gives `demo.customer2`/`demo.customer3` a few fresh `Completed` orders (fixed GUIDs, reconciled like point 4/9 rather than gated on "fresh database only") against three previously-untouched packages. `demo.customer3` deliberately stays opted **out** (`ShowOnLeaderboard = false`) despite also getting a real `Completed` order, so a fresh database's leaderboard visibly demonstrates the opt-in filter actually excluding someone with real order history — not just excluding people who coincidentally have none.
...
- **`Services/PackageServiceTests.cs`** — covers the Phase 8 additions: the bulk-action toolbar (`DuplicateManyAsync`/`AdjustQuantityManyAsync`/`ExtendPickupWindowManyAsync`, including that a manager not staffed to *every* affected package's business throws and saves nothing) and `GetForAnalyticsAsync`'s admin-vs-manager scoping, plus Phase 9's `HideAsync`/`UnhideAsync` authorization and state changes. `IPackageRepository`/`IBusinessService` are mocked, same shape as `BusinessServiceTests.cs`. **AI Phase 5**: `GetMarkdownCandidatesAsync`'s same admin-null/manager-scoped authorization shape as `GetForAnalyticsAsync`; `GetMarkdownSuggestionAsync` returning `null` without ever calling `IMarkdownPricingAgent` when the package doesn't exist, throwing on an unauthorized manager, and — the one test worth calling out — that the `PackageSellThroughRecord` history handed to the (mocked) agent is actually built correctly from a closed comparable package's real `OrderPackages`/`Order`/`Status` graph (`QuantitySold`/`QuantityOffered` computed correctly, not just passed through); `DismissMarkdownSuggestionAsync`'s authorization and that it actually sets `MarkdownDismissedAt`. `IMarkdownPricingAgent` is mocked alongside `IPackageRepository`/`IBusinessService`.
...
- **`Services/ReviewServiceTests.cs`** — `ReviewService`'s authorization (customer-only, only after a completed order, rating range) plus the package-tag validation in `GetContextAsync`/`SubmitAsync`: `ReviewablePackages` only populates once `CanReview` is true, and a `packageId` outside that set is silently dropped on submit rather than rejected. `IReviewRepository`/`IOrderRepository` are mocked.
- **`Services/PushSubscriptionServiceTests.cs`** — `PushSubscriptionService`'s authorization (`SubscribeAsync`/`UnsubscribeAsync` scoped to the signed-in caller, same shape as `FavoriteService`) and `SendToUserAsync`'s fan-out-then-prune behavior: every one of a user's subscriptions gets a send attempt, and only the ones `IWebPushGateway.SendAsync` reports as gone (410) get deleted. `IPushSubscriptionRepository`/`IWebPushGateway` are mocked.
- **`Services/NotificationServiceTests.cs`** — the one rule `CreateAsync` adds beyond inserting a row: it also calls `IPushSubscriptionService.SendToUserAsync` with the same message/url, for every caller (`OrderService`, `PackageService`, `BusinessService`...) without any of them needing their own push-specific code. `INotificationRepository`/`IPushSubscriptionService` are mocked.
- **`Services/BusinessTypeServiceTests.cs`** / **`Services/PackageTypeServiceTests.cs`** (Phase 11) — admin-only gate on every write, and the delete "in use" guard specifically: `IsInUseAsync` returning `true` must throw `InvalidOperationException` and never reach `DeleteAsync`/`SaveChangesAsync`. `IBusinessTypeRepository`/`IPackageTypeRepository`/`IAuditLogService` are mocked, same `FakeAuthenticationStateProvider`-backed `CurrentUserAccessor` shape as `BusinessServiceTests.cs`.
- **`Services/PackageAiAssistantTests.cs`** — `IChatClient` is mocked (Moq), never a real Ollama instance: covers the "no client configured → friendly `InvalidOperationException`" degradation (constructor's `IChatClient? chatClient = null` default), the happy path (`GetResponseAsync`'s `ChatResponse.Text` trimmed and returned), and that an exception from the chat client itself (e.g. connection refused) is rewrapped into the same friendly exception type rather than surfacing raw `OllamaSharp`/HTTP errors.
- **`Services/SearchIntentParserTests.cs`** (Phase 2) — same mocked-`IChatClient` shape as `PackageAiAssistantTests.cs` above, plus the JSON-parsing/validation specifics: a well-formed camelCase JSON response deserializes into the expected `SearchIntent`, a `dietaryTag` that isn't one of `Constants.DietaryTags.All` (e.g. a fabricated "Keto") is dropped to `null` rather than trusted, a negative `maxPrice` is dropped to `null`, and non-JSON model output throws the same friendly `InvalidOperationException` as a chat-client exception does. Manually verified once more against the real `qwen2.5:7b` model in `docker-compose.test.yml` (not part of the automated suite).
- **`Services/NearExpiryNudgeComposerTests.cs`** (Phase 3) — same mocked-`IChatClient` shape as `PackageAiAssistantTests.cs`/`SearchIntentParserTests.cs` above: the "no client configured" degradation, a trimmed happy-path response, and a chat-client exception rewrapped into the same friendly type. Manually verified end-to-end against the real `qwen2.5:7b` model via `docker-compose.test.yml` — the seeded "Late Save Meal Box" package (§9) produced a personalized nudge ("...matching your usual order...") for the demo customer and a plain one for an unmatched past customer, both delivered through the real bell notification and Mailpit email.
- **`Services/NearExpiryNudgeServiceTests.cs`** (Phase 3) — covers `NearExpiryNudgeService.SweepAsync`'s audience/grouping logic with `IPackageRepository`/`IFavoriteRepository`/`IOrderRepository`/`INotificationService`/`IAppEmailSender`/`INearExpiryNudgeComposer` all mocked: no candidates saves nothing, a candidate with no audience is still marked `NearExpiryNudgeSentAt` without calling the composer, a favoriter and a past customer are deduplicated into a single notification when they're the same user, a user with no email is notified but never emailed, and — the one case worth calling out — a customer whose completed-order history at that business shares a dietary tag with the package lands in its own group and triggers a second, separate `ComposeAsync` call from an unmatched customer's group, proving the "one AI call per distinct match, not per customer" batching actually groups correctly.
- **`Services/BasketPlannerAgentTests.cs`** (Phase 4) — unlike every other `Services/AI/` test file, the mocked `IChatClient` here sits *underneath* a real `FunctionInvokingChatClient` (`BasketPlannerAgent` wraps it internally via `.AsBuilder().UseFunctionInvocation().Build()`), so a mocked "model" turn that returns a `FunctionCallContent` genuinely invokes the real `search_live_packages` tool against a mocked `IPackageService` — only the chat responses themselves are fabricated (via `Mock<IChatClient>.SetupSequence`), the tool round-trip is real. Covers: no-client/invalid-input friendly errors (with `chatClient.Verify(..., Times.Never)` proving obviously-invalid input never even reaches the model); the happy path (tool call → real search → schema JSON referencing the real returned `packageId` → validated `BasketPlan`); a hallucinated `packageId` the tool never returned is silently dropped; a `Quantity` above real stock is clamped down; items spanning two businesses are trimmed to only the higher-`LineTotal` kitchen with a note appended to `Explanation`; a total over `budget` is trimmed from the end until it fits; and a chat-client exception rewraps into the friendly type. One test — `ProposeBasketAsync_BudgetTrimEmptiesBasket_ShowsNothingFitInsteadOfStaleExplanation` — exists purely because manual end-to-end testing against the real `qwen2.5:7b` model in `docker-compose.test.yml` found it failing: at an intentionally-too-low budget the model wrote an `Explanation` describing items that the budget trim then dropped entirely, so the UI showed a basket-shaped sentence next to an empty list. Manually verified end-to-end twice more against that real model: a "4 people, 30 lei" request that genuinely came back spanning two seeded kitchens' vegan packages was correctly trimmed to the higher-value kitchen (with the single-kitchen note shown), and approving the result actually added both packages to a real `CartService` basket.
- **`Services/MarkdownPricingAgentTests.cs`** (AI Phase 5) — same "mocked `IChatClient` sits underneath a real `FunctionInvokingChatClient`" shape as `BasketPlannerAgentTests.cs`, except the tool here is a closure over a plain in-memory `history` list rather than a mocked service call, so a mocked "model" turn that returns a `FunctionCallContent` genuinely invokes the real `get_sell_through_history` tool against that list. Covers: no-client friendly error; the happy path (tool call → schema JSON → validated `MarkdownSuggestion` with the real `CurrentPrice`/`SuggestedPrice`/`Explanation`); a `suggestedPrice` at or above the current price returns `null` (no cut warranted); a `suggestedPrice` below `MarkdownSettings.MinPriceFraction` (30%) of the current price is clamped up to that floor rather than trusted; a non-positive `suggestedPrice` returns `null`; and a chat-client exception rewraps into the friendly type. Manually verified end-to-end against the real `qwen2.5:7b` model in `docker-compose.test.yml`: the seeded "Away Day Meal Box" (§9) — priced at 12.99 RON against this business's own real 9.50–9.99 RON `mealBox` sell-through history — came back with a real tool call followed by "Cut to 10.99 RON — similar packages sold better at a lower price," and dismissing the suggestion on `/packages` persisted across a page reload.
- **`Services/ImageUploadServiceTests.cs`** — the one service test that touches real disk instead of mocking everything: `IWebHostEnvironment.WebRootPath` is pointed (via Moq) at a per-test temp directory, so `SaveAsync` writes real files there rather than needing a fake filesystem abstraction. Covers a valid upload landing under the right `{subfolder}` with a URL matching the saved file, two uploads never colliding on filename, and a disallowed extension throwing `InvalidOperationException` without creating the `uploads/` directory at all.
- **`Repositories/BusinessRepositoryTests.cs`** — the many-to-many `BusinessStaff` CRUD, the `SetHoursAsync`/`AddClosureAsync`/`RemoveClosureAsync` hours/closures methods, and the `dietaryTag` (Phase 10) and `maxPrice` (Phase 2) filters on `GetPagedAsync` — both proven the same way: only a business with a matching *live* package is returned, and an otherwise-matching but expired package is ignored — against a real `EcoMealDbContext`, InMemory-provider-backed (`InMemoryDb.Create()`) rather than mocked — exercises `BusinessRepository`'s own query/persistence logic instead of `BusinessService`'s authorization, which `BusinessServiceTests.cs` above covers with the repository mocked out.
- **`Repositories/OrderRepositoryTests.cs`** (Phase 11) — `GetTopRescuersAsync` against a real InMemory-provider `EcoMealDbContext`: only a `Completed` order inside the given date range from a user with `ShowOnLeaderboard == true` should count, proven by seeding a `Confirmed` order, an out-of-range `Completed` order, and an opted-out user's `Completed` order that should all be excluded from the result.
